...
class Parser:
    ...
    def parse_loop_report_list(self, loop_nest_struct):
        ...
        while True:
            ...
            if token.token_class == TokenClass.SKIP:
                ...
                logging.debug('top-level LOOP BEGIN ' + token.filename + '(' + str(token.line) + ')')
                ...
                sys.exit("error: parser: SKIP or LOOP BEGIN tokens are expected")
    ...
    def parse_loop_report(self, outer_main_loop):
        ...
        logging.debug('parse_loop_report(): loop at ' + loop.filename + '(' + str(loop.line) + ')')
        ...
        while True:
            ...
            if token.token_class == TokenClass.SKIP:
                ...
                logging.debug('inner LOOP BEGIN ' + token.filename + '(' + str(token.line) + ')')
                if token.inlined == True:
                    logging.debug('skipping inlined loop')
                    self.skip_loop()
                else:
